...
def handler(request, response):
    try:
        ...
        api_response = session.get(TARGET_API_URL, timeout=10)
        ...
        n_success = sum(
            1 for r in validation_results["results"] if r["success"]
        )
        n_failed = n_expectations - n_success
        percent_valid = (
            (n_success / n_expectations * 100) if n_expectations > 0 else 100
        )
